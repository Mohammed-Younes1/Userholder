...
function Pagination({ totalPosts, postsPerPage, currentPage, setCurrentPage }) {
    ...
    return (
        <nav aria-label="Page navigation example "className="flex pl-[175px] py-10">
                ...
                {pages.map(page => (
                    <li key={page} aria-current="page">
                        <button
                            ...
                            className={`relative block rounded bg-transparent px-3 py-1.5 text-sm transition-all duration-300 ${currentPage === page ? 'text-black font-bold' : 'text-neutral-600 dark:text-gray-400'} hover:bg-neutral-100 dark:hover:bg-gray-700 dark:hover:text-white`}
                        >
                            {page}
                        </button>
                    </li>
